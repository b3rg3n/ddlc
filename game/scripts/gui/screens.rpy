    ...
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="File error: \"characters/sayori.chr\"\n\nThe file is missing or corrupt.",
                ok_action=Show(screen="dialog", message="Файл сохранения поврежден. Начать новую игру.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="Сохраняться больше не нужно.\nЯ никуда не денусь, не волнуйся.", ok_action=Hide("dialog"))
        else:
            ...
            hbox:
                ...
                if config.developer:
                    vbox:
                        ...
                        textbutton _("Влево") action Preference("rollback side", "left")
                        textbutton _("Вправо") action Preference("rollback side", "right")
...
screen name_input(message, ok_action):
    ...
    frame:
        ...
        hbox:
            ...
            textbutton _("Да") action ok_action
