label ch0_main:
    ...
    "Тем не менее я лишь вздохнул и притормозил перед пешеходным переходом, позволяя [persistent.sayoriname] меня догнать."
    ...
    mc "Как скажешь, [persistent.sayoriname]..."
    s 1q "Хе-хе-хе..."
    ...
    "[persistent.sayoriname] любит перегибать с заботой обо мне, тогда как меня абсолютно устраивает то, что я трачу своё свободное время в основном на игры и аниме."
    s 4j "А-ага!"
    ...
    mc "Клубы..."
    "[persistent.sayoriname] хочет, чтобы я обратил на них внимание."
    "Думаю, у меня нет выбора, кроме как начать с клуба аниме..."

    s "Приве-е-ет!"
    show sayori 1b zorder 2 at t11
    mc "[persistent.sayoriname]?.."
    "[persistent.sayoriname], должно быть, зашла в класс, пока я витал в облаках."
    ...
    "[persistent.sayoriname] — вице-президент литературного клуба."
    ...
    "Даже не берусь утверждать, то ли [persistent.sayoriname] действительно настолько бестолковая, то ли достаточно хитрая, чтобы всё это спланировать."
    ...
    "Я подавленно следовал за [persistent.sayoriname] — сначала по коридорам, а потом вверх по лестнице, в то крыло, которое я посещал редко, поскольку в нём располагались только секции и учебные классы выпускников."
    "[persistent.sayoriname] ураганом ворвалась в класс."
    ...
    y "[persistent.sayoriname] всегда хорошо о тебе отзывалась."
    ...
    "И, если верить словам [persistent.sayoriname], та, кто пёк кексы."

    show sayori 2q zorder 3 at f31
    s "Можешь просто игнорировать её, когда она начинает капризничать!"
    "Прошептала [persistent.sayoriname] мне на ухо, а затем повернулась к остальным."
    ...
    "Юри, которая выглядела более взрослой и застенчивой, кажется, с трудом уживалась с такими людьми, как [persistent.sayoriname] и Нацуки."
    show yuri zorder 2 at t33
    ...
    "Как и сказала [persistent.sayoriname], за столом было одно место рядом с ней и одно рядом с Моникой."
    ...
    n "Разбирайте их уже!"
    "Первый взяла [persistent.sayoriname], за ней — Моника. Я последовал их примеру."
    show natsuki zorder 2 at t32
    show sayori zorder 3 at f31
    s 4q "Вкусно!"
    ...
    mc "М-да? А я думаю, что, фактически, так. [persistent.sayoriname] сказала..."
    show natsuki zorder 2 at t32
    ...
    "Что-то мне подсказывало, что не стоит говорить Монике о том, что [persistent.sayoriname] буквально притащила меня сюда."
    mc "Ну, я пока не числюсь ни в одном из клубов, а [persistent.sayoriname], похоже, очень нравится здесь, так что..."
    ...
    "[persistent.sayoriname] села за спиной Нацуки и положила руки ей на плечи."
    show natsuki at h42
    n 1v "{i}Я не милая!!{/i}"
    ...
    mc "[persistent.sayoriname], может, и уговорила меня заглянуть, но я не принимал окончательного решения."
    ...
    "Верно. Мы с [persistent.sayoriname] перестали ходить домой вместе, потому что она всегда оставалась после занятий в каком-то клубе."
